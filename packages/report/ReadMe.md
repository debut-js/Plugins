...
figures | Array<{ name: string; getValue: () => number; fill: boolean; type: FigureType; }> | Массив линий, поле `name` - описывает название линии на графике, метод `getValue` - возвращает актуальное значение индикатора на данный момент времени, `fill` - позволяет выполнить заливку по типу `FillType` либо к оси, либо до линии ниже, `FigureType` поддерживает типы линия и bar.
...
setXRange | Задание диапазона значений для оси X, позволяет отрезать начало и конец. Принимает значения времени с и по какое производить обрезку.
